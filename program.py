    ...
    if not to_do:
        ...
        for j in to_do:
            ...
            if mark == "Y" or mark == "Yes":
                complete[j] = [to_do[j], "Completed"]
            elif mark == "N" or mark == "No":
                complete[j] = [to_do[j], "Not Completed"]
            else:
                print("Invalid option. Choose Y or N")

        with open("completed.csv", "a", newline='') as ca:
            with open("uncompleted.csv", "a", newline='') as ua:
                csv_writer_ca = csv.writer(ca, delimiter=' ')
                csv_writer_ua = csv.writer(ua, delimiter=' ')
                for i in complete:
                    if complete[i][1] == "Completed":
                        csv_writer_ca.writerow([i, complete[i][0], complete[i][1]])
                    elif complete[i][1] == "Not Completed":
                        csv_writer_ua.writerow([i, complete[i][0], complete[i][1]])
    ...
    try:
        ...
        if check == 1:
            ...
            if not os.path.exists(complete_file):
                ...
                with open(complete_file, "r", newline='') as cr:
                    csv_reader_cr = csv.reader(cr, delimiter=' ')
                    for row in csv_reader_cr:
                        name_, due_date_, complete_ = row
                        print(f"{name_} {due_date_} {complete_}")
        elif check == 2:
            uncomplete_file = "uncompleted.csv"
            if not os.path.exists(uncomplete_file):
                print("No task marked as uncomplete.")
            else:
                with open(uncomplete_file, "r", newline='') as ur:
                    csv_reader_ur = csv.reader(ur, delimiter=' ')
                    for row in csv_reader_ur:
                        name__, due_date__, uncompleted__ = row
                        print(f"{name__} {uncompleted__} has a due date of {due_date__}")
        else:
